...
@@ -120,6 +120,9 @@ static void _sock_bind_wild(int sockfd)
...
+		sin.ss_len, sin.ss_family, sin.__ss_align);
 	for (retry=0; retry < PORT_RETRIES ; retry++) {
 		...
 		if (rc >= 0)
@@ -422,6 +425,10 @@ extern int slurm_init_msg_engine(slurm_addr_t *addr)
 		goto error;
 	}
...
@@ -669,6 +676,7 @@ extern void slurm_set_addr(slurm_addr_t *addr, uint16_
